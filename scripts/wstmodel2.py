"""
Train a simple NN with the windsat data AND
TELSEM emissivity atlas (converted to netcdf)
...
params.add_argument(
    "--source_folder", default="./data/raw/daily_Windsat/",
    help="Folder with Windsat dataset."
)

params.add_argument(
    "--output_folder", default="./models/",
    help="Folder to store final weights and trining history."
)

params.add_argument(
    "--telsem_folder", default="./data/processed/WindsatEmiss/",
    help="Folder with the TELSEM emissivities in .nc format"
)

params.add_argument(
    "--swath_sector", default=0,
    help="Ascending pass = 0, Descending pass = 1."
)
...
if __name__ == "__main__":
    ...
    swath_ds = model_preprocess(
        ws_ds, swath_sector=swath_sector,
        look="impute", add_look_flag=False
    )
    d_vars = [
        "surtep_ERA5",
        "lat",
    ...
    combined_df["lon"] = combined_df["lon"].apply(
        lambda x: np.sin(np.deg2rad(x))
    )
    combined_df["lat"] = combined_df["lat"].apply(
        lambda x: np.sin(np.deg2rad(x))
    )
    ...
    checkpoints = ModelCheckpoint(
        filepath=os.path.join(
            output_folder, f"checkpoint_{swath2char[swath_sector]}.keras"
        ),
        verbose=1
    )
