...
def histogram_matrix(h5file, save_path=None):
    ...
    for i, field in enumerate(fields):
        if field != 'lat' & field != 'lon':
            axs[i // col_number, i % col_number].hist(h5file[field][:].flatten(), bins=100)
...
def column_plot(xarray_dataset, save_path = None):
    ...
    nrows = len(xarray_dataset.time) // 2 + 1 if len(xarray_dataset.time) % 2 == 1 else len(xarray_dataset.time) // 2
