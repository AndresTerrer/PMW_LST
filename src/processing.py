...
def apply_scaling(xarray_dataset: xr.Dataset) -> xr.Dataset:
    ...
    """
    scaled_dataset = xarray_dataset.copy()  # Create a copy to avoid modifying the original dataset
...
def load_zip(path)-> xr.Dataset:
    ...
    with zipfile.ZipFile(path, 'r') as zip_file:
        ...
        default_date = datetime(2016, 12, 31)  # Set to one day earlier than the first valid date
...
def create_landmask(xarray_dataset, threshold = 273, show = False, file_path = None, figure_path = None) -> xr.DataArray:
    """
